...
def register_commands(tree, bot):
    ...
    async def clearcontext(interaction: discord.Interaction):
        try:
            ...
            print(f"⚠️ Could not defer interaction: {e}")
        cleared_files = 0
        for f in CACHE_DIR.glob("*.lb01"):
            try:
                ...
                print(f"⚠️ Failed to delete cache file {f}: {e}")
    ...
    async def joke(interaction: discord.Interaction):
        ...
        try:
            ...
            print(f"❌ Error fetching joke from AI: {e}")
            joke_text = "😵‍💫 Oops! Something went wrong while fetching your joke."
